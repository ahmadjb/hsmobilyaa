...
const EditingCloakroom = () => {

    const [data, setData] = useState([]);
    const [txtcloakroom, setBedrooms] = useState("");
    ...
    const openImageModal = (imageUrl) => {
        setImgCloakroom(imageUrl);
        setEyeClicked(true);
    };

    const closeImageModal = () => {
        setImgCloakroom(null);
        setEyeClicked(false);
    };
    ...
    const fetchData = async () => {
        ...
        try {
            const valRef = collection(txtDB, 'cloakroom'); // Change 'doors' to your collection name
    ...
    useEffect(() => {
        ...
        const fetchDataForEditing = async () => {
            ...
            try {
                if (editingId) {
                    const valRef = doc(collection(txtDB, 'cloakroom'), editingId);
                    const docSnap = await getDoc(valRef);
                    if (docSnap.exists()) {
                        const data = docSnap.data();
                        setBedrooms(data.txtval);
                        setImgCloakroom(data.imgUrl);
                    }
                }
            } catch (error) {
        ...
        if (txtcloakroom === "" && imgCloakroom === "") {
            ...
            try {
                ...
                if (id) {
                    ...
                    if (txtcloakroom !== "") {
                        updateData.txtval = txtcloakroom;
                    }

                    if (imgCloakroom !== "") {
                        updateData.imgUrl = imgCloakroom;
                    }
                    ...
                    alert(`Data with ID ${id} updated successfully in 'cloakroom' collection`);
    ...
    return (
        ...
        <div className='text-admin-1' style={{ paddingTop: 30 }}>
            ...
            {imgCloakroom && eyeClicked && (
                <ModalPhoto imageUrl={imgCloakroom} onClose={closeImageModal} />
            )}
        </div>
    );
};

export default EditingCloakroom;
